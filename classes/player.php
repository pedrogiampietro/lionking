...
class Player extends ObjectData
{
    ...
    public function makeOutfitUrl($class = NULL, $direction = NULL)
    {
        ...
        return "<img {$c} src='http://outfits.ferobraglobal.com/animoutfit.php?id={$this->getLookType()}&addons={$this->getLookAddons()}&head={$this->getLookHead()}&body={$this->getLookBody()}&legs={$this->getLookLegs()}&feet={$this->getLookFeet()}&mount={$this->getLookMount()}{$d}' alt='{$this->getName()}' name='{$this->getName()}'>";
    }
    ...
    public function getVocationName()
    {
        ...
        switch ($voc) {
            ...
            case 8:
                ...
                break;
            case 9:
                $voc = "Masterful Sorcerer";
                break;
            case 10:
                $voc = "Ancient Druid";
                break;
            case 11:
                $voc = "Glorious Paladin";
                break;
            case 12:
                $voc = "Rageful Knight";
                break;
    ...
    */
     ...
     * @param $value
     */
    public function setHideItem($value)
    {
        ...
        if (empty($result)) {
            $createTable = Website::getDBHandle()->prepare("ALTER TABLE `players` ADD COLUMN `hide_char_items` TINYINT(1) NOT NULL DEFAULT 0 AFTER `hide_char`");
            $createTable->execute([]);
        }
