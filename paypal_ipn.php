 ...
 * @return bool
 */
$doubleStatus = function ($price, $qtdcoins) use ($config) {

    $promo = $qtdcoins;
    if($config['site']['promoactive']){
        foreach ($config['site']['promovalues'] as $key => $value){
            $promo = ($price >= $key? $value*$qtdcoins : $promo);
        }
    }
    return $promo;
};
    ...
    if ($verified) {
        ...
        if (!$transactionCompleted($tid)) {
            if ($payment_status == "Completed") {
                ...
                $acc->setPremiumPoints($acc->getPremiumPoints() + $doubleStatus($price, $coinCount));
                $acc->save();
